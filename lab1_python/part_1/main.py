...
encoding = arguments.encoding.value  # кодировка
...
with open(file_path, 'r') as file_input, open(file_path_out, 'w') as file_output:
    ...
    while counter < number_of_str:
        rand_str = create_random_str_hash(random.randint(
            size_of_string_begin, size_of_string_end))
        file_output.write(rand_str + '\n')
        counter += 1
